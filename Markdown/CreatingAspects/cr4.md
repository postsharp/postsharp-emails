...
In an earlier email we demonstrated a very simple example of logging, noting the name of a method being called.. Now we're going to delve deeper into logging and in the process describe more of the options that are available to you when creating your own aspects.

In this example we'll log not just the method name being called but also any parameters (with their type) that are being passed into it and any return value if relevant.

For the time being we'll continue to output the messages to the console as strings and to facilitate their creation we'll create an interpolated string builder that we can use in our aspect.
...
The code above is relatively straightforward but there a couple of points worth noting. The first is the use of the special 'meta' keyword that enables us to access the elements in our source code. The second point to be noted is that Metalama cannot read the value of out parameters. This is entirely logical given that these would not be known at the time that the code is actually compiled.

Now that we have out InterpolatedStringBuilder we can create our revised logging aspect that will utilise it.
